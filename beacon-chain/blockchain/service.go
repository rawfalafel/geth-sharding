...
// persisted to the db.
func (c *ChainService) HasStoredState() bool {
	return c.beaconDB.HasInitialState()
}
...
func (c *ChainService) SaveBlock(block *types.Block) error {
	return c.beaconDB.SaveBlock(block)
}
...
// in the db.
func (c *ChainService) CheckForCanonicalBlockBySlot(slotNumber uint64) bool {
	return c.beaconDB.HasBlockForSlot(slotNumber)
}
...
func (c *ChainService) GetCanonicalBlockBySlotNumber(slotNumber uint64) (*types.Block, error) {
	return c.beaconDB.GetBlockBySlot(slotNumber)
}
...
func (c *ChainService) blockProcessing(done <-chan struct{}) {
	...
	for {
		...
		case block := <-c.incomingBlockChan:
			...
			parentExists := c.beaconDB.HasBlock(block.ParentHash())
